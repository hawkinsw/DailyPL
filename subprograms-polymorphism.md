...
2.  _parametric polymorphism_: A type of polymorphism where subprograms take an implicit/explicit type parameter used to define the types of their subprogram's parameters. In parametric polymorphism, no matter the value of the type parameter, the subprogram's semantics are always the same.​

Ad-hoc polymorphism is sometimes called function overloading ([C++](https://en.cppreference.com/w/cpp/language/overload_resolution.html), [Typescript](https://www.typescriptlang.org/docs/handbook/2/functions.html#function-overloads)). Subprograms that participate in ad-hoc polymorphism share the same name but must have different protocols.[^c++protocols] If the subprograms' protocols and names were the same, how would the compiler/interpreter choose which one to invoke? Although a subprogram's protocol includes its return type, not all languages allow ad-hoc polymorphism to depend on the return type (e.g., C++). See the various definitions of `add` in the C++ code here: [https://github.com/hawkinsw/cs3003/blob/main/subprograms/subprograms.cpp](https://github.com/hawkinsw/cs3003/blob/main/subprograms/subprograms.cpp). Note how they all have different protocols. Further, note that not all the versions of the function `add` perform an actual addition! That's the point of _ad-hoc polymorphism_ -- the programmer can change the meaning of a function.

[^c++protocols]: Not that C++ does _not_ consider every element in a function's protocol when looking at a function call and resolving which version of the versions to execute. In particular, C++ does not consider the return type in doing overloaded function resolution.
