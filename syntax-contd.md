...
Previously we worked with a grammar that purported to describe all the strings that looked like natural numbers:

![](./graphics/NaturalGrammarTerminologyLabeled.png)
...
![](./graphics/BadNaturalGrammar.png)

If our grammar _does_ actually generate all the strings that represent natural numbers, we have just proven (again, by construction) that $012$ is an natural number. This seems funny. A number that starts with $0$ (that is not just a $0$) should _not_ be deemed a natural number. Let's see how we can fix this problem.

In order to guarantee that we cannot derive a number that contain more than one digit and starts with a $0$, we will need to add a few more productions. First, let's reconsider the production for the start symbol $Nat$. At the very highest level, we know that $0$ is an natural number. So, our start symbol's production should handle that case.

With the base case accounted for, we next consider that an natural number greater than $0$ cannot start with a zero, but it can start with any other digit between $1$ and $9$. It seems handy to have a production for a non-terminal that expands to the terminals $1$ through $9$. We will call that non-terminal $Nzd$ for _non-zero digits_. After the initial non-zero digit, a natural number can contain zero or more digits between $0$ and $9$ (we can call this the $Rest$ of the natural number). For brevity, we probably want a production that will expand to all the digits between $0$ and $9$. Let's call that non-terminal $Zd$ for zero digits. We'll define it either as a $0$ or anything in $Nzd$. If we put all of this together, we get the following grammar:

![](./graphics/GoodNaturalGrammar.png)

Let's look at a few derivations to see if this gets the job done. Is $0$ an natural number?

![](./graphics/NatGrammarDeriveZero.png)

And a single-digit natural number?

![](./graphics/NatGrammarDeriveNine.png)

How about an natural number between 10 and 100, inclusive?

![](./graphics/NatGrammarDeriveTen.png)

We are really cooking. Let's try one more. Is a number greater than $100$ derivable?

![](./graphics/NatGrammarDeriveOneHundredFiftyFour.png)
...
Until now we have used proof-by-construction through derivations to show that a particular string is in the language described by a particular grammar. What's really cool is that any derivation can also be written in the form of a tree. The two representations of a derivation contain the same information -- a proof that a particular sentence is in a language defined by a certain grammar. Let's look at the derivation of the expression `1 + 5 * 2`:
