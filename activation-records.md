...
In addition to restoring `multiply_by_two`'s stack frame to its position of importance before transferring execution control back to the point where it called `add`, the return value needs to be setup for delivery to `multiply_by_two`, too.
...
Please, _please_, do yourself a favor and look at the last figure in [More Turtles](#more-turtles). What do you see? Exactly! They are *nearly* identical. And, what's cooler? That the next instruction that the CPU is about to execute is the instruction right after the `call` that kicked off `add`. If `multiply_by_two` didn't know better, it would have absolutely no idea that the CPU secretly transported to the land of the person answering our letter and helped them write their response!

> Note: "We" are `multiply_by_two` again, and the postal worker has just handed us the letter sent back by the person from `add`land to whom we mailed our letter.
...
essentially releases the memory allocated during the execution of the code emitted for the prologue of `multiply_by_two`'s. Pretty sneaky!
...
And _now_ it's our chance to rip open the envelope and see what the post officer brought us from the land of `add`:

```asm
        mov     DWORD PTR [ebp-4], eax
```

Remember that `add` put its result into `eax`? Well, between the time that the result was put into that register and now, no one has changed the register's value. So, when we perform the `mov` operation above, the code emitted by the compiler is simply instructing the CPU to store the result of `add` into a place in memory -- the place at $4$ bytes below the `ebp`. 

Remember back to the description earlier where we discussed how it was possible to determine that `ebp-4` was the address of `result` in `add`?. Notice that the code we just analyzed (code in th epilogue of `multiply_by_two`, mind you!) is identical! In other words, it seems like a reasonable conclusion that this operation is storing the result of `add` into a local variable just like the same operation in `add` was storing the result of the `+` into a local variable. And, because there is only one local variable in `multiply_by_two`, we know that the storage for the local variable `multiplied` is $4$ bytes beneath the base pointer! 
...
At this point, the cycle begins again. The next instructions are the ones that the compiler emitted for the epilogue of `multiply_by_two`. If we traced the execution of those final few instructions, we would see that the purple stackframe is methodically dismantled and control would return to the instruction after the place in `main` where `multiply_by_two` was called. I encourage you to do this analysis yourself and see where it ends up!
